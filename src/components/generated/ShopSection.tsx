...
const merchandise = [{
  ...
  mpid: "a82e6fa1-2d0a-4c92-8da6-54efdbf285a9"
}, {
  id: "training-hoodie",
  ...
  mpid: "9e895085-9100-4b7c-8e08-efec4973f826"
}, {
  id: "boxing-gloves",
  ...
  mpid: "1913e97f-636a-4ee2-8049-f8b5c0300fba"
}, {
  id: "gym-bag",
  ...
  badge: "",
  colors: ["Black", "Red"],
  mpid: "a3325368-f266-4654-9361-79f713b45800"
}, {
  id: "fight-shorts",
  ...
  mpid: "b307f249-8512-47fa-8be6-de8e4cb41a48"
}, {
  id: "water-bottle",
  ...
  mpid: "a476b814-6e1e-4424-aa0c-442e58d54a57"
}] as any[];
export default function ShopSection({
  className = ""
}: ShopSectionProps) {
  return <section className={`py-20 bg-white ${className}`} id="shop" data-magicpath-id="6" data-magicpath-path="ShopSection.tsx">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" data-magicpath-id="7" data-magicpath-path="ShopSection.tsx">
        {/* Header */}
        <div className="text-center mb-16" data-magicpath-id="8" data-magicpath-path="ShopSection.tsx">
          <Badge variant="outline" className="mb-4 text-sm font-medium" data-magicpath-id="9" data-magicpath-path="ShopSection.tsx">
            Official Merchandise
          </Badge>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6" data-magicpath-id="10" data-magicpath-path="ShopSection.tsx">
            Champion's Collection
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed" data-magicpath-id="11" data-magicpath-path="ShopSection.tsx">
        ...
        {/* Features Bar */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12" data-magicpath-id="12" data-magicpath-path="ShopSection.tsx">
          <div className="flex items-center justify-center p-4 bg-gray-50 rounded-lg" data-magicpath-id="13" data-magicpath-path="ShopSection.tsx">
            <Truck className="w-6 h-6 text-red-600 mr-3" data-magicpath-id="14" data-magicpath-path="ShopSection.tsx" />
            <span className="text-sm font-medium text-gray-700" data-magicpath-id="15" data-magicpath-path="ShopSection.tsx">Free shipping over $75</span>
          </div>
          <div className="flex items-center justify-center p-4 bg-gray-50 rounded-lg" data-magicpath-id="16" data-magicpath-path="ShopSection.tsx">
            <Shield className="w-6 h-6 text-red-600 mr-3" data-magicpath-id="17" data-magicpath-path="ShopSection.tsx" />
            <span className="text-sm font-medium text-gray-700" data-magicpath-id="18" data-magicpath-path="ShopSection.tsx">30-day return guarantee</span>
          </div>
          <div className="flex items-center justify-center p-4 bg-gray-50 rounded-lg" data-magicpath-id="19" data-magicpath-path="ShopSection.tsx">
            <Star className="w-6 h-6 text-red-600 mr-3" data-magicpath-id="20" data-magicpath-path="ShopSection.tsx" />
            <span className="text-sm font-medium text-gray-700" data-magicpath-id="21" data-magicpath-path="ShopSection.tsx">Authentic merchandise only</span>
          </div>
        </div>
          ...
          {merchandise.map(item => <Card key={item.id} className="group overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="23" data-magicpath-path="ShopSection.tsx">
              <div className="relative overflow-hidden" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="24" data-magicpath-path="ShopSection.tsx">
                ...
                <Button size="sm" className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white text-black hover:bg-gray-100" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="27" data-magicpath-path="ShopSection.tsx">
                  <ShoppingCart className="w-4 h-4" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="28" data-magicpath-path="ShopSection.tsx" />
                </Button>
              </div>
              ...
              <CardContent className="pt-0" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="35" data-magicpath-path="ShopSection.tsx">
                ...
                <div className="flex items-center justify-between mb-4" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="37" data-magicpath-path="ShopSection.tsx">
                  ...
                  <div className="flex space-x-1" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="41" data-magicpath-path="ShopSection.tsx">
                    {item.colors.map((color, index) => <div key={index} className={`w-6 h-6 rounded-full border-2 border-gray-300 ${color.includes('Black') ? 'bg-black' : color.includes('White') ? 'bg-white' : color.includes('Red') ? 'bg-red-600' : color.includes('Gray') ? 'bg-gray-500' : color.includes('Navy') ? 'bg-blue-900' : color.includes('Silver') ? 'bg-gray-400' : color.includes('Gold') ? 'bg-yellow-500' : color.includes('Green') ? 'bg-green-600' : color.includes('Orange') ? 'bg-orange-600' : 'bg-purple-600'}`} title={color} data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="42" data-magicpath-path="ShopSection.tsx" />)}
                  </div>
                </div>

                <Button className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold" data-magicpath-uuid={(item as any)["mpid"] ?? "unsafe"} data-magicpath-id="43" data-magicpath-path="ShopSection.tsx">
                  Add to Cart
                </Button>
              </CardContent>
            </Card>)}
        ...
        {/* Newsletter Signup */}
        <div className="bg-gradient-to-r from-red-600 to-red-700 rounded-2xl p-12 text-white text-center" data-magicpath-id="44" data-magicpath-path="ShopSection.tsx">
          <h3 className="text-3xl font-bold mb-4" data-magicpath-id="45" data-magicpath-path="ShopSection.tsx">
            Stay Updated on New Releases
          </h3>
          <p className="text-xl text-red-100 mb-8 max-w-2xl mx-auto" data-magicpath-id="46" data-magicpath-path="ShopSection.tsx">
            Be the first to know about new merchandise, exclusive drops, and special offers from Kumar's collection.
          </p>
          <div className="flex flex-col sm:flex-row max-w-md mx-auto gap-4" data-magicpath-id="47" data-magicpath-path="ShopSection.tsx">
            <input type="email" placeholder="Enter your email" className="flex-1 px-4 py-3 rounded-lg text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-white" data-magicpath-id="48" data-magicpath-path="ShopSection.tsx" />
            <Button className="bg-white text-red-600 hover:bg-gray-100 font-semibold px-8" data-magicpath-id="49" data-magicpath-path="ShopSection.tsx">
              Subscribe
            </Button>
          </div>
        </div>
